...
export function StatusChip({ title, value, active = false, accent = 'signal', icon }: StatusChipProps) {
  ...
  return (
    <div className={`inline-flex min-w-[110px] items-start justify-between gap-3 border px-3 py-2 ${accentClass}`}>
      <div className="flex flex-col">
        <span className="font-mono text-[9px] font-semibold uppercase tracking-[0.18em] text-white/55">
          {title}
        </span>
        <span className="font-mono text-[12px] font-bold uppercase tracking-[0.16em]">{value}</span>
      </div>
