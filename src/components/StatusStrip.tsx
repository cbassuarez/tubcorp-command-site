...
export function StatusStrip({ telemetry }: StatusStripProps) {
  ...
  return (
    <div className="fixed left-0 right-0 top-0 z-40 border-b border-line bg-surface-overlay backdrop-blur-sm">
      <motion.div
      ...
      >
        ...
        <StatusChip
          ...
          icon={<Lock size={13} />}
        />
        <TubCorpWordmark size="sm" />
        <div className="ml-auto hidden items-center gap-2 md:flex">
          <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-txt-muted">SOURCE</span>
          <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-txt">{sourceLabel}</span>
        </div>
      </motion.div>
    </div>
  )
